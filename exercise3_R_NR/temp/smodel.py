...
class Model (object):
	...
	def __init__(self, history_length, learning_rate):
		...
		conv3_flat = tf.reshape(conv3, [-1, 24*24*32])
		...
		dropout2 = tf.layers.dropout(inputs=dense2, rate=0.7)
	...
	def training (self, X_train, y_train, X_valid, y_valid, epochs, batch_size, model_dir):
		...
		with self.sess as sess:
			...
			for epoch in range(epochs):
				...
				for i  in range(x_train_batchsize):
					...
					y_batch = id_to_action(y_batch)
					#batch_correct_count = sess.run(accuracy,feed_dict={self.x:x_batch, self.y_: y_batch})
				...
				##	batch_correct_count = sess.run(accuracy,feed_dict={self.x_input:x_val, self.y_label: y_val})
				##	correctsum_val += batch_correct_count
				##	valid_cost[epoch] += self.sess.run(self.cost, feed_dict={self.x_input:x_val, self.y_label:y_val})
